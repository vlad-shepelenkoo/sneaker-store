    ...
    <div class="main-content-wrap">
        ...
        <form class="tf-section-2 form-add-product" method="POST" enctype="multipart/form-data" action="{{route('admin.product.update')}}">
            ...
            <div class="wg-box">
                ...
                <div class="mb10">
                    <div id="create-sizes-button" class="body-title mb-10">
                        <a class="body-title tf-button">Add Size</a>
                    </div>
                    <div class="mb10">
                        <form>                                
                            <div id="sizes" class="mb10">
                                @foreach ($itemSizes as $size)
                                    <div class="cols gap22">
                                        <input name="sizes[item{{\Carbon\Carbon::parse($size->created_at)->timestamp + $size->id}}][size]" type="text" value="{{$size->size}}" class="mb-10 mt-10" placeholder="Size" />
                                        <input name="sizes[item{{\Carbon\Carbon::parse($size->created_at)->timestamp + $size->id}}][quantity]" type="text" value="{{$size->quantity}}" class="mb-10 mt-10" placeholder="Quantity" />
                                    </div>
                                @endforeach
                            </div>
                        </form>
    ...
    <script>
        $(function(){
            ...
            $("input[name='name']").on("change", function(){
                $("input[name='slug']").val(StrinToSlug($(this).val()));
            });
        });

        function StrinToSlug(Text){
        ...
        var sizes = 1;
        function createSizeComponent(){
            var rootElement = document.createElement('div');
                rootElement.setAttribute('class', 'row');

            var contents = `<div class="cols gap22">
                                <input name="sizes[item`+sizes+`][size]" type="text" class="mb-10 mt-10" placeholder="Size" />
                                <input name="sizes[item`+sizes+`][quantity]" type="text" class="mb-10 mt-10" placeholder="Quantity" />
                            </div>`;
        ...
        function onClickAddSizeButton(event){
            var button = event.target,
                container = document.querySelector('#sizes'),
        ...
        var button = document.getElementById('create-sizes-button');
        button.addEventListener('click', onClickAddSizeButton);
    </script>
@endpush